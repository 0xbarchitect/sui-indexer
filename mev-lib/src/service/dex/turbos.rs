...
#[async_trait]
impl DEXService for TurbosService {
    ...
    async fn get_pool_data(&self, pool_id: &str) -> Result<crate::types::Pool> {
        ...
        let pool_data =
            serde_json::from_value::<TurbosPool>(pool_fields.to_json_value()).map_err(|e| {
                error!("Failed to deserialize pool fields: {}", e);
                e
            })?;
        info!("Turbos deserialized: {:?}", pool_data);
...
impl TurbosService {
    fn format_onchain_pool(
        ...
        let current_tick_index = tick_math::i32_from_u32(pool.tick_current_index)?;
        ...
        Ok(crate::types::Pool {
            ...
            fee_rate: Some(pool.fee as i32),
            is_pause: Some(!pool.unlocked),
            fees_swap_in: None,
            fees_swap_out: None,
        })
    }

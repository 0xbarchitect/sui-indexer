...
#[async_trait]
impl DEXService for FlowXService {
    ...
    async fn get_pool_data(&self, pool_id: &str) -> Result<crate::types::Pool> {
        ...
        let coin_types = crate::utils::get_coin_types_from_pool_type(&pool_type, &self.exchange)?;
        ...
        let pool_data =
            serde_json::from_value::<FlowXPool>(pool_fields.to_json_value()).map_err(|e| {
                error!("Failed to deserialize pool fields: {}", e);
                e
            })?;
        info!("FlowX deserialized: {:?}", pool_data);
